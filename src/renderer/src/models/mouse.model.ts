  ...
  private onMouseWheelScroll(event: WheelEvent) {
    ...
    if (cumulativeScrolling) {
      ...
      if (Math.abs(this.scroll_delta.x) >= 100) {
        delta.x = (this.scroll_delta.x / Math.abs(this.scroll_delta.x)) * -DEFAULT_SCROLL_RATE
        this.scroll_delta.x = 0
      }

      this.scroll_delta.y += event.deltaY * factor // Vertical scrolling
      if (Math.abs(this.scroll_delta.y) >= 100) {
        delta.y = (this.scroll_delta.y / Math.abs(this.scroll_delta.y)) * -DEFAULT_SCROLL_RATE
        this.scroll_delta.y = 0
      }
    } else {
      if (event.deltaX !== 0) {
        delta.x = (event.deltaX / Math.abs(event.deltaX)) * -DEFAULT_SCROLL_RATE
      }
      if (event.deltaY !== 0) {
        delta.y = (event.deltaY / Math.abs(event.deltaY)) * -DEFAULT_SCROLL_RATE
      }
    }
