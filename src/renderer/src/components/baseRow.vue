<template>
  <div
    ...
    :style="computedStyle.outer"
  >
    <div
      v-for="(outer, index) in computedItems"
      :key="index"
      class="row-line full-width"
      :style="computedStyle.inner"
    >
      <div v-for="(inner, indexInner) of outer" :key="indexInner" class="base-col">
        <div v-if="inner === EMPTY_ITEM"></div>
        <slot v-else :data="inner"></slot>
      </div>
    </div>
  </div>
</template>
...
const props = withDefaults(
  defineProps<{ countPerLine: number; items?: T[]; gutter?: number | [number, number] }>(),
  {
    gutter: 16,
    items: () => []
  }
