...
/** 解析HID状态 */
export function parseHidStatus (mouseOrKeyboardOnline: boolean, hidOnline: boolean, hidBusy: boolean, captured: boolean, enabled) {
    if (!enabled) {
...
export const keymapSelections = Object.entries(Keymaps).map(([, value]) => new SelectOptions(value, keymapLabelMap.get(value)))
